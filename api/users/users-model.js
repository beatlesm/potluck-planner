...
async function add({ username, password, role_name }) { // done for you
  ...
  await db.transaction(async trx => {
    ...
    if (role) {
      ...
      const [newRoleObject] = await trx('roles').insert({ role_name: role_name }, ['role_id', 'role_name'])
      role_id_to_use = newRoleObject.role_id
    }
    const [newUserObject] = await trx('users').insert({ username, password, role_id: role_id_to_use }, ['role_id', 'username', 'password'])
    created_user = newUserObject
  })
  return created_user
}
